...
static int send_data_only(const void *data, size_t len)
{
	...
	if (bin_len != sizeof(claim_token)) return -EINVAL;

	// Calculate header size: 14 bytes (Hardwario header) + 1 byte (Message Type = 0x00)
	size_t header_len = 15;
	uint8_t *send_buffer = k_malloc(header_len + len);
	if (!send_buffer) return -ENOMEM;

	// 15th byte: Payload Type (0x00)
	send_buffer[14] = 0x00;

	// Copy CBOR data
	memcpy(send_buffer + header_len, data, len);

	// Construct header (without hash first)
	// Bytes 8-11: Serial Number
	sys_put_be32(serial_number, send_buffer + 8);
	
	// Bytes 12-13: Flags (0xC = FIRST | LAST) and Sequence Number
	...
	// Bytes 0-7: Hash calculation
	uint8_t packet_hash[8];
	// Hash is over everything after the 8-byte hash field
